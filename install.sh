...
check_token() {
  ...
  if [ "$USER_TOKEN" = "TirzzDev" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di TirzzNesia${NC}"
    echo -e "${YELLOW}TELEGRAM : @zexore4l${NC}"
    echo -e "${YELLOW}WHATSAPP : +628975919600${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© TirzzNesia${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  while true; do
    ...
    case "$SELECT_THEME" in
      1)
        THEME_URL=$(echo -e "https://github.com/TirzzNesia/Pterodactyl-Theme-Autoinstaller/raw/main/stellar.zip")        
        break
        ;;
      2)
        THEME_URL=$(echo -e "https://github.com/TirzzNesia/Pterodactyl-Theme-Autoinstaller/raw/main/billing.zip")
        break
        ;;
      3)
        THEME_URL=$(echo -e "https://github.com/TirzzNesia/Pterodactyl-Theme-Autoinstaller/raw/main/enigma.zip")
        break
        ;; 
      x)
        return
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer TirzzNesia   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : zexore4l      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : TirzzNesia  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by TirzzNesia  ${NC}"
